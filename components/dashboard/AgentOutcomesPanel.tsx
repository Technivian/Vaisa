...
export default function AgentOutcomesPanel() {
  ...
  return (
    <SectionPanel title={t.overview.agentOutcomes.title} description={t.overview.agentOutcomes.subtitle}>
      <div className="flex h-2.5 w-full overflow-hidden rounded-full">
        <div
          className={success.dot}
          style={{ width: `${AI_HANDLING.resolvedPercentage}%` }}
          title={`${t.overview.agentOutcomes.resolvedAutomatically} — ${AI_HANDLING.resolvedPercentage}%`}
        />
        <div
          className={warning.dot}
          style={{ width: `${AI_HANDLING.escalatedPercentage}%` }}
          title={`${t.overview.agentOutcomes.humanEscalation} — ${AI_HANDLING.escalatedPercentage}%`}
        />
      </div>
