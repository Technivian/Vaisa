    ...
    <div className="flex min-h-screen flex-col bg-surface md:flex-row">
      ...
      <aside className="sticky top-0 hidden h-screen w-60 shrink-0 flex-col bg-ink text-white md:flex">
        <div className="flex items-start justify-between gap-2 px-5 py-5">
          <div>
            ...
            <p className="text-xs text-white/40">{t.shell.workspace}</p>
          </div>
          <LanguageToggle />
        </div>
        ...
        <div className="space-y-3 border-t border-white/10 px-3 py-4">
          ...
          >
            {t.shell.openAssistant}
            <ConversationsIcon className="h-3.5 w-3.5" />
          </button>
          <div className="px-1 text-[11px] leading-relaxed text-white/35">
            <p className="font-medium text-white/45">{t.shell.demoEnvironment}</p>
            <p>{t.shell.simulatedAnalytics}</p>
            <p className="mt-1.5">{t.shell.conceptDisclaimer}</p>
          </div>
        </div>
      </aside>
      ...
      <header className="flex flex-col bg-ink text-white md:hidden">
        <div className="flex items-center justify-between px-4 py-3">
          <div>
            <p className="text-sm font-bold tracking-tight">VAISA</p>
            <p className="text-[11px] text-white/40">{t.shell.workspace}</p>
          </div>
